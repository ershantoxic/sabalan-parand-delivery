...
class Orders extends ConsumerWidget { const Orders({super.key}); Future<void> _refresh(WidgetRef ref) async { ref.invalidate(ordersProvider); await ref.read(ordersProvider.future); } @override Widget build(BuildContext context, WidgetRef ref) { final AsyncValue<List<Order>> orders = ref.watch(ordersProvider); return Scaffold(appBar: AppBar(title: const Text('سفارش‌های امروز')), body: RefreshIndicator(onRefresh: () => _refresh(ref), child: orders.when(data: (List<Order> list) => list.isEmpty ? ListView(children: const <Widget>[SizedBox(height: 250), Center(child: Text('سفارشی برای امروز ثبت نشده است.'))]) : ListView.builder(itemCount: list.length, itemBuilder: (BuildContext context, int index) => OrderCard(list[index])), error: (_, __) => ListView(children: const <Widget>[SizedBox(height: 250), Center(child: Text('ارتباط با سرور برقرار نشد. دوباره تلاش کنید.'))]), loading: () => const Center(child: CircularProgressIndicator())))); } }
